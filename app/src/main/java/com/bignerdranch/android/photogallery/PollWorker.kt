...
class PollWorker(
    ...
    override suspend fun doWork(): Result {
        ...
        if (query.isEmpty()) {
            Log.i(TAG, "doWork: no saved query, finishing early")
        ...
        return try {
            ...
            if (items.isNotEmpty()) {
                ...
                if (newResultId == lastId) {
                    Log.i(TAG, "doWork: still the same result $newResultId")
                } else {
                    Log.i(TAG, "doWork: Got a new result $newResultId")
            ...
            Log.e(TAG, "doWork: background update failed", e)
            Result.failure()
        }
    }
